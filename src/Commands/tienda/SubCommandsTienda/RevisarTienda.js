  ...
  async execute(interaction, client) {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "No hay artículos disponibles en la tienda. Regresa mas tarde"
            )
            .setTimestamp(),
        ],
        ephemeral: true,
    ...
    const mainEmbed = EmbedBuilder.from(baseEmbed).setDescription(
      "```md\n# Bienvenido a la Tienda```\n" +
        ...
        `> 🏷️ Artículos Únicos: \`${
          new Set(tiendaData.Inventario.map((item) => item.Articulo)).size
        }\`\n\n` +
        "```md\n# Información```\n" +
        ...
        .setDescription(
          "```md\n# Artículos Disponibles```\n" +
            ...
            `# Página ${Math.floor(i / itemsPerPage) + 1} de ${Math.ceil(
              tiendaData.Inventario.length / itemsPerPage
            )}` +
            "```"
        );
