...
export default {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option
        .setName("usuario")
        .setDescription("👤 Usuario del que quieres ver el inventario (opcional)")
    ),
  ...
  async execute(interaction, client) {
    ...
    const mainEmbed = EmbedBuilder.from(baseEmbed)
      .setDescription(
        ...
        "**📊 Estadísticas**\n" +
        `> 📦 Total de Artículos: \`${inventarioData.Inventario.reduce((acc, item) => acc + item.Cantidad, 0)}\`\n` +
    ...
    // Ordenar inventario por fecha de compra (más reciente primero)
    ...
    for (let i = 0; i < sortedInventario.length; i += itemsPerPage) {
      ...
      const pageEmbed = EmbedBuilder.from(baseEmbed)
        .setDescription(
          ...
          itemsDisplay +
          "\n```md\n" +
          `# Página ${Math.floor(i / itemsPerPage) + 1} de ${Math.ceil(sortedInventario.length / itemsPerPage)}` +
          "```"
        );
